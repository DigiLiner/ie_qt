...
def draw_line(img1: QImage, pt1: QPoint, pt2: QPoint, task: str):
    """
    Draws a point or a line with a precise softness (blur) and opacity effect.
    Uses QPainterPath and gradient brush for soft edges without alpha accumulation.
    """
    ...
    # Alpha birikmesini önlemek için Source kompozisyon modu
    painter.setCompositionMode(QPainter.CompositionMode.CompositionMode_SourceOver)
    ...
    # Eğer softness yoksa normal çiz
    if softness == 0:
        painter.setPen(ie_globals.current_pen)
        if task == "down":
            painter.drawPoint(pt1)
    ...
    # --- Yeni Yaklaşım: QPainterPath ile Gradient Dolgu ---
    
    # Çizgi kalınlığı ve softness ayarları
    total_width = pen_width + (softness / 100.0) * pen_width * 0.5
    
    # QPainterPath oluştur
    path = QPainterPath()
    
    if task == "down":
        # Nokta için daire çiz
        path.addEllipse(pt1, total_width/2, total_width/2)
    elif task == "move":
        # Çizgi için yuvarlak uçlu path
        path.moveTo(pt1)
        path.lineTo(pt2)
    
    # Gradient brush oluştur (soft edge efekt için)
    if softness > 0:
        # Radial gradient için
        if task == "down":
            gradient = QRadialGradient(pt1, total_width/2)
        else:
            # Linear gradient for lines (daha karmaşık, alternatif yaklaşım)
            # Basitçe solid brush kullanıp dışarıya doğru transparency
            gradient = QRadialGradient(QPointF(0, 0), total_width/2)
        
        # Gradient renkleri - merkezde tam opak, kenarlarda şeffaf
        center_color = QColor(base_color)
        center_color.setAlphaF(base_alpha)
        
        edge_color = QColor(base_color)
        edge_color.setAlphaF(0.0)
        
        gradient.setColorAt(0.0, center_color)
        gradient.setColorAt(1.0 - (softness/200.0), center_color)  # Softness kontrolü
        ...
        if task == "move":
            # Çizgi için kalınlık ayarı
            pen = QPen(QBrush(gradient), total_width)
            pen.setCapStyle(Qt.PenCapStyle.RoundCap)
            pen.setJoinStyle(Qt.PenJoinStyle.RoundJoin)
            painter.setPen(pen)
    ...
    buffer = QImage(img1.size(), QImage.Format.Format_RGBA64)
