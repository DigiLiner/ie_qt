...
def draw_pen(img1: QImage, start_pos: QPoint, end_pos: QPoint):
    """
    Pen aracı için özel çizim metodu - alpha birikmesini önler
    """
    ...
    # Fare hareket hızına göre çizim sıklığını ayarla
    step_size =int( max(1.0, length / 5.0))

    # Çizgi boyunca interpolasyon
    steps = int(length / step_size) + 1
    old_x :int= start_pos.x()
    old_y :int= start_pos.y()
    for i in range(steps):
        t = i / steps
        x = int( start_pos.x() * (1 - t) + end_pos.x() * t)
        y = int(start_pos.y() * (1 - t) + end_pos.y() * t)

        # Her adımda yeni bir buffer kullan
        buffer = QImage(img1.size(), img1.format())
        ...
        # Alpha değerini azalt (birikmeyi önlemek için)
        color = QColor(ie_globals.current_pen.color())
        original_alpha = color.alphaF()
        color.setAlphaF(original_alpha * 0.3)  # Alpha'yı %30 azalt
        ...
        buffer_painter.drawLine(QPoint(int(x), int(y)), QPoint(int(old_x), int(old_y)   ))
        old_x = x
        old_y = y

        buffer_painter.end()

        # Buffer'ı ana resme karıştır
        final_painter = QPainter(img1)
        final_painter.setCompositionMode(QPainter.CompositionMode.CompositionMode_SourceOver)
        final_painter.drawImage(0, 0, buffer)
        final_painter.end()
...
def draw_round_rect(img1: QImage,  pt1:QPoint, pt2:QPoint,task:str,corner_radius:int=10) -> None:
    ...
    # Use the same format as the target image
    temp_img = QImage(clipped_rect.size(), img1.format())
    temp_img.fill(Qt.GlobalColortransparent)
