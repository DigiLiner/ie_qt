...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def setSvgColors(self):
        ...
        def set_icon(button_obj):
            if not isinstance(button_obj, (QtWidgets.QToolButton, QtWidgets.QMenu)):
                return
            
            icon = QIcon()
            
            object_name = button_obj.objectName()
            if "toolButton" in object_name:
                iconname = object_name.replace("toolButton", "").lower()
            elif "menu" in object_name:
                iconname = object_name.replace("menu", "").lower()
            else:
                return

            icon_relative_path = os.path.join("resources", "themes", themename, f"{iconname}.svg")
            icon_absolute_path = res resource_path(icon_relative_path)
            
            if os.path.exists(icon_absolute_path):
                icon.addFile(icon_absolute_path, QSize(), QIcon.Mode.Normal, QIcon.State.Off)
                button_obj.setIcon(icon)

        # Iterate through all relevant widgets and set their icons
        for container in [self.dwTools, self.menuBar()]:
            for obj in container.findChildren((QtWidgets.QToolButton, QtWidgets.QMenu)):
                set_icon(obj)
