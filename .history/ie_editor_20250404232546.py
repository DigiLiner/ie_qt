...
#region mouse wheel [rgba(222, 100, 222,0.1)]
    def pic1_mouseWheelEvent(self, event: QtGui.QWheelEvent) -> None:
        ...
        image_x_before = (hbar.value() + event_pos.x()) / self.zoomFactor
        image_y_before = (vbar.value() + event_pos.y()) / self.zoomFactor
        ...
        # Calculate offset
        x_offset = round(event_pos.x() - ((event_pos.x() /self.zoomFactor)))
        y_offset = round(event_pos.y() - ((event_pos.y() /self.zoomFactor)))

        # Update widget size (this changes scroll bar ranges)
        self.pic1_update()

        # New position of the image pixel under the mouse
        new_image_x = image_x_before * self.zoomFactor
        new_image_y = image_y_before * self.zoomFactor

        # Center scroll to keep pixel under the mouse
        hbar.setValue(int(new_image_x - event_pos.x()))
        vbar.setValue(int(new_image_y - event_pos.y()))
