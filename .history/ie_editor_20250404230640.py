...
#region mouse wheel [rgba(222, 100, 222,0.1)]
    ...
    mouse_pos_image_before = QPointF(
        (hbar.value() + mouse_pos_viewport.x()) / self.zoomFactor,
        (vbar.value() + mouse_pos_viewport.y()) / self.zoomFactor
    )
    ...
    mouse_pos_image_after = QPointF(
        mouse_pos_image_before.x() * self.zoomFactor,
        mouse_pos_image_before.y() * self.zoomFactor
    )
    
    # Calculate new scroll positions to keep mouse position stable
    new_hbar_value = mouse_pos_image_after.x() - mouse_pos_viewport.x()
    new_vbar_value = mouse_pos_image_after.y() - mouse_pos_viewport.y()
