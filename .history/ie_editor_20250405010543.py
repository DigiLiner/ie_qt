...
##region mouse wheel [rgba(222, 100, 222,0.1)]
    def pic1_mouseWheelEvent(self, event: QtGui.QWheelEvent) -> None:
        ...
        # Get the widget's position in the viewport
        widget_pos = self.widgetPicture1.mapFrom(self.scrollArea.viewport(), viewport_pos.toPoint())
        
        # Calculate point in original image coordinates
        img_x = widget_pos.x() / self.zoomFactor
        img_y = widget_pos.y() / self.zoomFactor
        ...
        new_x = int(img_x * new_zoom - viewport_pos.x())
        new_y = int(img_y * new_zoom - viewport_pos.y())
        
        # Apply new scroll positions
        hbar.setValue(new_x)
        vbar.setValue(new_y)
        print(f"Mouse viewport: {viewport_pos.x()}, {viewport_pos.y()}")
print(f"Widget pos: {widget_pos.x()}, {widget_pos.y()}")
print(f"Image pos: {img_x}, {img_y}")
print(f"New scroll: {new_x}, {new_y}")
#endregion
