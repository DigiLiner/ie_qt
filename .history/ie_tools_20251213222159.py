...
def draw_line_pixel_perfect(img1: QImage, pt1: QPoint, pt2: QPoint, task: str):
    # ... kontroller ...
    
    # RGBA64 formatında pixel verilerini al
    img1.convertTo(QImage.Format.Format_RGBA64_Premultiplied)
    
    # Çizim için gerekli parametreler
    base_color = ie_globals.current_pen.color()
    pen_width = ie_globals.current_pen.widthF()
    softness = ie_globals.pen_blur
    
    # Bresenham çizgi algoritması veya anti-aliased çizgi
    # Her pixel için maksimum alpha değerini kullan
    
    if task == "down":
        # Nokta çiz
        x, y = int(pt1.x()), int(pt1.y())
        radius = max(1, int(pen_width / 2))
                ...
                if dist <= radius:
                    ...
                    if 0 <= px < img1.width() and 0 <= py < img1.height():
                        ...
                        if softness > 0:
                            ...
                            alpha_mult = falloff * (softness / 100.0)
                        else:
                            alpha_mult = 1.0 if dist <= pen_width/2 else 0.0
                        
                        # Mevcut pixeli al
                        pixel_color = img1.pixelColor(px, py)
                        
                        # Sadece daha yüksek alpha değeri varsa güncelle
                        new_alpha = base_color.alphaF() * alpha_mult
                        if new_alpha > pixel_color.alphaF():
                            # Alpha değerini güncelle (biriktirme)
                            result_color = QColor(base_color)
                            result_color.setAlphaF(new_alpha)
                            img1.setPixelColor(px, py, result_color)
    
    elif task == "move":
        # Çizgi çiz (basitleştirilmiş)
        x1, y1 = int(pt1.x()), int(pt1.y())
        x2, y2 = int(pt2.x()), int(pt2.y())
        
        # Wu's anti-aliased line algorithm veya benzeri kullanılabilir
        ...
        for i in range(steps + 1):
            ...
            # Her adımda nokta çiz (yukarıdaki nokta çizim mantığı)
            # ... nokta çizim kodu ...
