...
#region mouse wheel [rgba(222, 100, 222,0.1)]
    def pic1_mouseWheelEvent(self, event: QtGui.QWheelEvent) -> None:
        ...
        print("Bars", hbar.value(),vbar.value())
        # Farenin widget içindeki gerçek konumu
        widget_pos_x = eventpos.x() + hbar.value()
        widget_pos_y = eventpos.y() + vbar.value()
        print(widget_pos_x, widget_pos_y)
        # Original image position
        old_zoom = self.zoomFactor
        img_pos_x = widget_pos_x / old_zoom
        img_pos_y = widget_pos_y / old_zoom
        ...
        new_widget_pos_x = widget_pos_x * self.zoomFactor
        new_widget_pos_y = widget_pos_y * self.zoomFactor
        
        # Scroll pozisyonlarını ayarla - fare konumu sabit kalacak şekilde
        new_scroll_x = new_widget_pos_x - eventpos.x()
        new_scroll_y = new_widget_pos_y - eventpos.y()
        #new_scroll_x = hbar.value() + (new_widget_pos_x - widget_pos_x)
        #new_scroll_y = vbar.value() + (new_widget_pos_y - widget_pos_y)
...
#region mouse press [rgba(255, 255, 121,0.1)]
    ...
    def pic1_mouseWheelEvent(self, event: QtGui.QWheelEvent) -> None:
        ...
        # Calculate the mouse position relative to the image (pre-zoom)
        img_pos_x = (hbar.value() + mouse_pos.x()) / self.zoomFactor
        img_pos_y = (vbar.value() + mouse_pos.y()) / self.zoomFactor
        ...
        # Calculate where the mouse position should be after zoom
        new_mouse_x = img_pos_x * self.zoomFactor
        new_mouse_y = img_pos_y * self.zoomFactor
        
        # Adjust scrollbars to keep mouse position stable
        new_hbar_value = new_mouse_x - mouse_pos.x()
        new_vbar_value = new_mouse_y - mouse_pos.y()
        
        # Set new scrollbar positions
            
                                        
        hbar.setValue(int(new_hbar_value))
        vbar.setValue(int(new_vbar_value))
