...
#region mouse wheel [rgba(222, 100, 222,0.1)]
    ...
    # Current scrollbar values
        ...
        # Mouse position relative to viewport (float)
        event_pos = event.position()

        # Save old zoom and calculate zoom change
        ...
        zoom_ratio = self.zoomFactor / old_zoom

        # Scroll offsets before zoom
        scroll_x = hbar.value()
        scroll_y = vbar.value()

        # Mouse position in full image space (before zoom)
        mouse_x_img = scroll_x + event_pos.x()
        mouse_y_img = scroll_y + event_pos.y()

        # Call update to resize the image widget
        self.pic1_update()

        # New scroll values to keep the mouse point centered
        new_mouse_x = mouse_x_img * zoom_ratio
        new_mouse_y = mouse_y_img * zoom_ratio

        hbar.setValue(int(new_mouse_x - event_pos.x()))
        vbar.setValue(int(new_mouse_y - event_pos.y()))
