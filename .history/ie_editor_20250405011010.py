...
##region mouse wheel [rgba(222, 100, 222,0.1)]
    ...
    # Get current scroll positions and print debug info
    scroll_x = hbar.value()
    scroll_y = vbar.value()
    print("\n=== Zoom Debug Info ===")
    print(f"Current scroll position: ({scroll_x}, {scroll_y})")
    
    # Get mouse position relative to viewport
    viewport_pos = event.position()
    print(f"Viewport mouse position: ({viewport_pos.x():.1f}, {viewport_pos.y():.1f})")
    
    # Calculate absolute position in the image
    abs_x = (viewport_pos.x() + scroll_x) / self.zoomFactor
    abs_y = (viewport_pos.y() + scroll_y) / self.zoomFactor
    print(f"Image space position: ({abs_x:.1f}, {abs_y:.1f})")
    print(f"Current zoom factor: {self.zoomFactor:.3f}")
    
    # Calculate new zoom factor
    if event.angleDelta().y() > 0:
        new_zoom = self.zoomFactor * ie_globals.zoomInFactor
        print(f"Zooming IN by factor: {ie_globals.zoomInFactor}")
    else:
        new_zoom = self.zoomFactor * ie_globals.zoomOutFactor
        print(f"Zooming OUT by factor: {ie_globals.zoomOutFactor}")
    
    # Clamp zoom factor
    new_zoom = max(0.1, min(10.0, new_zoom))
    print(f"New zoom factor: {new_zoom:.3f}")
    ...
    new_scroll_x = int(abs_x * new_zoom - viewport_pos.x())
    new_scroll_y = int(abs_y * new_zoom - viewport_pos.y())
    print(f"New scroll position: ({new_scroll_x}, {new_scroll_y})")
    
    # Calculate expected point under cursor after zoom
    expected_x = (new_scroll_x + viewport_pos.x()) / new_zoom
    expected_y = (new_scroll_y + viewport_pos.y()) / new_zoom
    print(f"Point under cursor after zoom: ({expected_x:.1f}, {expected_y:.1f})")
    print(f"Point should match original: ({abs_x:.1f}, {abs_y:.1f})")
