...
import ie_globals,ie_editor, ie_filters
from main_ui import Ui_MainWindow
from float_window_ui import Ui_floatWindow as float_window_ui
from dialog_newImage_ui import Ui_Dialog as Ui_NewImageDialog
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def new_file(self):
        dialog = Ui_NewImageDialog()
        dialog_instance = QtWidgets.QDialog()
        dialog.setupUi(dialog_instance)
        dialog.comboBoxSizeList.addItems(["100x100", "500x500", "800x600","1024x768","1280x1024","1600x1200","1920x1080","2048x1536"])
        dialog.comboBoxSizeList.setCurrentIndex(3)
        dialog.plainTextEditWidth.setPlainText("1024")
        dialog.plainTextEditHeight.setPlainText("768")

        def on_size_change(index):
            sizes = ["100x100", "500x500", "800x600","1024x768","1280x1024","1600x1200","1920x1080","2048x1536"]
            size = sizes[index]
            width, height = size.split('x')
            dialog.plainTextEditWidth.setPlainText(width)
            dialog.plainTextEditHeight.setPlainText(height)

        dialog.comboBoxSizeList.currentIndexChanged.connect(on_size_change)

        if dialog_instance.exec() == QtWidgets.QDialog.accepted:
            width = int(dialog.plainTextEditWidth.toPlainText())
            height = int(dialog.plainTextEditHeight.toPlainText())

            EditorType = cast(Type[QWidget], ie_editor.Editor)
            doc = ie_editor.Editor("Picture" + str(ie_globals.filenamecounter),"Picture" + str(ie_globals.filenamecounter), width, height)
            self.tabWidget.addTab(doc, "Picture" + str(ie_globals.filenamecounter))
            ie_globals.filenamecounter += 1
            self.tabWidget.setCurrentIndex(self.tabWidget.count()-1)
    def undo(self):
