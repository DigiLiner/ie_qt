...
class Editor(draw_window_ui.Ui_Form, QWidget):
    ...
    def pic1_mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if ie_globals.current_tool == ie_globals.ie_tool_pen:
                ...
                previous_brush_alpha = ie_globals.current_brush.color().alpha()
                ...
                ie_globals.current_brush.color().setAlpha(previous_brush_alpha)
    ...
    def pic1_mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if event.buttons() == Qt.MouseButton.LeftButton:
            ...
            # BAŞLANGIÇ NOKTASINI DEĞİL, LASTPOS'U KULLAN
            virtualLastPos = QPoint(
                math.trunc(self.lastPos.x() / self.zoomFactor),
                math.trunc(self.lastPos.y() / self.zoomFactor)
            )
            ie_globals.statusText.pos = "Mouse Position: " + str(virtualpos.x()) + ", " + str(virtualpos.y())

            if ie_globals.current_tool == ie_globals.ie_tool_pen:
                # Fare hareket hızını kontrol et
                dist = math.hypot(virtualpos.x() - virtualLastPos.x(),
                                  virtualpos.y() - virtualLastPos.y())

                # Çok kısa hareketlerde çizim yapma
                print(dist)
                if dist < 0.5:
                    self.lastPos = event.pos()
                    return
                ...
                # Sadece lastPos'u güncelle, startPos'u değil
                self.lastPos = event.pos()
